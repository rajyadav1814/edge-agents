/**
 * Test script for Gemini 1.5 Pro using the Google Generative AI SDK
 */
...
const model = genAI.getGenerativeModel({
  model: "gemini-1.5-pro", // Using gemini-1.5-pro instead
});
console.log("Initialized model: gemini-1.5-pro");
...
async function run() {
  console.log("Starting content generation with Gemini 1.5 Pro...");
  
  try {
    // Generate content directly (not using chat)
    console.log("Sending prompt to model...");
    const result = await model.generateContent({
      contents: [{ role: "user", parts: [{ text: "What are the main differences between Gemini and GPT models? Please provide a short answer." }] }],
      generationConfig,
    });
