...
describe('Message Validator', () => {
  ...
  describe('validateMessageType', () => {
    test('should validate client_hello message', () => {
      ...
      const result = messageValidator.validateMessageType(message);
      expect(result.valid).toBe(true);
      expect(result.errors).toEqual([]);
    });
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle undefined message', () => {
      const result = messageValidator.validateMessage(undefined);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Message must be an object');
    });
    
    test('should handle non-object message', () => {
      const result = messageValidator.validateMessage('not-an-object');
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Message must be an object');
    });
  ...
  describe('Validation Helpers', () => {
    ...
    test('validateRequiredFields should return errors for missing fields', () => {
      ...
      const errors = messageValidator.validateRequiredFields(obj, requiredFields);
