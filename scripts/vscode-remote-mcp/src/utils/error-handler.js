 ...
 */
function handleError(error, handlers) {
  const { disconnectAndNotifyUser, logWarningAndContinue, implementRecoveryAction } = handlers;
  ...
  // Only use recovery actions that are explicitly provided in the error object
  if (error && error.payload && error.payload.recoveryAction) {
