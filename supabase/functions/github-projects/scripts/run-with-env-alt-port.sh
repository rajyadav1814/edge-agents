#!/bin/bash
# Run GitHub API server with environment variables from .env file on an alternative port
...
echo -e "\n${YELLOW}Creating temporary server file with alternative port...${NC}"
TEMP_FILE="index-temp.ts"

# Copy the index-test.ts file
cat index-test.ts > $TEMP_FILE

# Replace the port in the serve function call
sed -i "s/serve(handleRequest)/serve(handleRequest, { port: $ALT_PORT })/g" $TEMP_FILE

# Run the server
echo -e "\n${GREEN}Starting GitHub API server on port $ALT_PORT...${NC}"
echo "Press Ctrl+C to stop the server"
echo -e "${YELLOW}Server will be available at http://localhost:$ALT_PORT/github-api/${NC}\n"
